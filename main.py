...
POSTGRES_CONFIG = { "POSTGRES_HOST": os.getenv("POSTGRES_HOST", "34.245.131.114"), "POSTGRES_PORT": os.getenv("POSTGRES_PORT", "5432"), "POSTGRES_DATABASE": os.getenv("POSTGRES_DATABASE", "oiai_database_dev"), "POSTGRES_USER": os.getenv("POSTGRES_USER", "dev"), "POSTGRES_PASSWORD": os.getenv("POSTGRES_PASSWORD", "*&jkahg345jk"), }
...
def parse_student_query(prompt: str, token: str, allowed_metrics: List[str]) -> Dict[str, object]:
    ...
    guidance = (
        ...
        "- completion, progress, percent -> completion_rate\n"
        "- completed -> completed_items\n"
        "- topics covered, topic count -> topics_covered\n"
        "- enrollments, courses -> enrollments\n"
        "- activity, events, engagement -> activity_events\n"
        "- study time, time spent -> total_study_time_minutes\n"
...
def get_available_metrics(conn) -> Dict[str, Dict[str, object]]:
    ...
    if table_exists(conn, "public", "activity_performance"):
        ...
        if "cur_progress" in cols and "max_progress" in cols:
            metrics["completion_rate"] = {
                "metric": "completion_rate",
                "table": "public.activity_performance",
                "agg": (
                    "COALESCE("
                    "SUM(cur_progress) * 100.0 / NULLIF(SUM(max_progress), 0), 0"
                    ")"
                ),
            ...
            metrics["completed_items"] = {
                ...
                "table": "public.activity_performance",
                "agg": (
                    "COUNT(*) FILTER (WHERE cur_progress = max_progress AND max_progress > 0)"
                ),
        ...
        if "topic_id" in cols:
            metrics["topics_covered"] = {
                ...
                "table": "public.activity_performance",
                "agg": "COUNT(DISTINCT topic_id)",
                "alias": "topics_covered",
            }

        if "total_time_spent_seconds" in cols:
            metrics["total_study_time_minutes"] = {
                "metric": "total_study_time_minutes",
                "table": "public.activity_performance",
                "agg": "COALESCE(SUM(total_time_spent_seconds) / 60.0, 0)",
                "alias": "total_study_time_minutes",
            }
...
def build_student_summary_prompt(
    ...
    prompt = (
        ...
        "Prioritize learning progress, completion rate, study time, and notable engagement patterns. "
        ...
        f"Recent progress records (sample):\n{df_preview(progress_df)}\n\n"
...
def fetch_users(conn, search: str = "", limit: int | None = None) -> pd.DataFrame:
    ...
    query += sql.SQL(" ORDER BY {} DESC NULLS LAST").format(sql.Identifier(order_col))
...
def safe_progress_metrics(conn, user_id: int) -> Dict[str, float | int | None]:
    result = {
        "completion_rate": None,
        "completed_items": None,
        "topics_covered": None,
        "total_study_time_minutes": None,
    }

    if not table_exists(conn, "public", "activity_performance"):
        return result

    cols = set(get_column_names(conn, "public", "activity_performance"))
    needed = {"user_id", "cur_progress", "max_progress"}
    if not needed.issubset(cols):
        return result

    completed_expr = "NULL::bigint AS completed_items"
    topics_expr = "NULL::bigint AS topics_covered"
    time_expr = "NULL::double precision AS total_study_time_minutes"

    if "cur_progress" in cols and "max_progress" in cols:
        completed_expr = (
            "COUNT(*) FILTER (WHERE cur_progress = max_progress AND max_progress > 0) AS completed_items"
        )
    if "topic_id" in cols:
        topics_expr = "COUNT(DISTINCT topic_id) AS topics_covered"
    if "total_time_spent_seconds" in cols:
        time_expr = "COALESCE(SUM(total_time_spent_seconds) / 60.0, 0) AS total_study_time_minutes"

    query = f"""
        SELECT
            COALESCE(SUM(cur_progress) * 100.0 / NULLIF(SUM(max_progress), 0), 0) AS completion_rate,
            {completed_expr},
            {topics_expr},
            {time_expr}
        FROM public.activity_performance
        WHERE user_id = %s
    """
    ...
    with conn.cursor() as cur:
        ...
        row = cur.fetchone()
        if row:
            result["completion_rate"] = float(row[0]) if row[0] is not None else None
            result["completed_items"] = int(row[1]) if row[1] is not None else None
            result["topics_covered"] = int(row[2]) if row[2] is not None else None
            result["total_study_time_minutes"] = float(row[3]) if row[3] is not None else None
...
def render_student_profiles(conn):
    ...
    st.write("Review student activity across enrollments, learning, and chat.")
    ...
    progress_metrics = safe_progress_metrics(conn, user_id)
    ...
    metrics = [
        ...
        ("Completion Rate (%)", progress_metrics["completion_rate"]),
        ("Completed Items", progress_metrics["completed_items"]),
        ("Topics Covered", progress_metrics["topics_covered"]),
        ("Study Time (min)", progress_metrics["total_study_time_minutes"]),
    ]

    metric_cols = st.columns(len(metrics))
    for col, (label, value) in zip(metric_cols, metrics):
        precision = 1 if isinstance(value, float) else 0
        col.metric(label, format_metric_value(value, precision))
                ...
                with st.spinner("Generating summary..."):
                    try:
                        ...
                        progress_df = fetch_recent_rows(
                            ...
                            where_col="user_id",
                            where_val=user_id,
                            order_candidates=["updated_at", "created_at", "id"],
                            limit=10,
                        )
    ...
    with st.expander("Smart Student Finder", expanded=False):
        query_text = st.text_area(
            ...
            placeholder="Show students with completion rate above 70 and topics covered over 3",
        )
    ...
    tabs = st.tabs(["Profile", "Enrollments", "Progress", "Chats"])
    ...
    with tabs[2]:
        ...
        st.subheader("Recent Progress Records")
        progress_df = fetch_recent_rows(
            conn,
            "public",
            "activity_performance",
        ...
        if progress_df.empty:
            st.info("No progress activity found.")
        else:
            st.dataframe(progress_df, use_container_width=True, height=360)
...
def prepare_engagement_signals(conn) -> List[Dict[str, object]]:
    signal_defs = [
        ...
        },
        {
            ...
            "date_candidates": ["updated_at", "created_at"],
            "metric_cols": ["total_time_spent_seconds", "cur_progress", "max_progress"],
        },
        {
            "key": "ai_chat",
            "label": "AI Chats",
...
def fetch_signal_timeseries(
    ...
    if "cur_progress" in column_set and "max_progress" in column_set:
        select_parts.append(
            sql.SQL(
                "COALESCE(SUM(cur_progress) * 100.0 / NULLIF(SUM(max_progress), 0), 0) AS completion_rate"
            )
...
def render_engagement_insights(conn):
    ...
    st.write("Trend activity signals across learning, progress, and chat.")
    ...
    for tab, signal in zip(tabs, signals):
        with tab:
            ...
            avg_users = df["users"].mean() if "users" in df.columns else None

            time_col = None
            for candidate in ["time_spent", "total_time_spent_seconds"]:
                if candidate in df.columns:
                    time_col = candidate
                    break

            total_time = df[time_col].sum() if time_col else None
            ...
            metric_row[2].metric("Total Time", format_metric_value(total_time, 1))
